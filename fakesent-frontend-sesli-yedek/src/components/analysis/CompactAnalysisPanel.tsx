...
export function CompactAnalysisPanel() {
  ...
  return (
    <div style={{ padding: "1.5rem", color: theme.colors.textPrimary, background: panelBg, borderRadius: "8px", border: `1px solid ${panelBorder}`, transition: "all 0.3s ease", boxShadow: "0 4px 12px rgba(0,0,0,0.5)", fontFamily: "sans-serif" }}>
      ...
      <div>
        <ModalityScoreBar label="Ses ↔ Dudak Uyumu" score={Math.round(currentSnapshot.scores.audioLip)} status={getModalityStatus(currentSnapshot.scores.audioLip)} />
        <ModalityScoreBar label="Ses ↔ Yüz Uyumu" score={Math.round(currentSnapshot.scores.audioFace)} status={getModalityStatus(currentSnapshot.scores.audioFace)} />
        <ModalityScoreBar label="Yüz ↔ Dudak Uyumu" score={Math.round(currentSnapshot.scores.faceLip)} status={getModalityStatus(currentSnapshot.scores.faceLip)} />
      </div>
